...
const styles = StyleSheet.create({
  ...
  logo: { width: 130, height: 57 },
  orderBlock: { alignItems: 'flex-end' },
  orderTitle: { fontSize: 20, fontFamily: 'Helvetica-Bold', color: C.dark },
  orderMeta:  { fontSize: 9, color: C.gray, marginTop: 3 },
  estadoBadge: {
    marginTop: 5, paddingVertical: 3, paddingHorizontal: 10,
    backgroundColor: C.light, borderRadius: 20,
    borderWidth: 1, borderColor: C.border, alignSelf: 'flex-end',
  },
  estadoText: { fontSize: 8, fontFamily: 'Helvetica-Bold', color: C.indigo },
  // ── Info box ──────────────────────────────────────────────────────────────
  infoBox: {
    backgroundColor: C.light, padding: 14, borderRadius: 6,
    marginBottom: 22, flexDirection: 'row', justifyContent: 'space-between',
    borderWidth: 1, borderColor: C.border,
  },
  infoLabel: { fontSize: 7, color: C.gray, marginBottom: 3, fontFamily: 'Helvetica-Bold', letterSpacing: 0.5 },
  infoValue: { fontSize: 11, fontFamily: 'Helvetica-Bold', color: C.dark },
  // ── Table ─────────────────────────────────────────────────────────────────
  sectionTitle: { fontSize: 8, fontFamily: 'Helvetica-Bold', color: C.gray, letterSpacing: 1.5, marginBottom: 8 },
  tableHeader: {
    flexDirection: 'row', backgroundColor: C.dark,
    paddingVertical: 8, paddingHorizontal: 10, borderRadius: 4, marginBottom: 2,
  },
  tableRow: {
  ...
  tdBold:   { fontSize: 9, fontFamily: 'Helvetica-Bold', color: C.dark },
  tdMono:   { fontSize: 8, color: C.indigo, fontFamily: 'Helvetica-Oblique' },
  tdGreen:  { fontSize: 9, fontFamily: 'Helvetica-Bold', color: C.green, textAlign: 'right' },
  productImg: { width: 46, height: 46, objectFit: 'cover' },
  imgBox:     { width: 46, height: 46, backgroundColor: C.light, justifyContent: 'center', alignItems: 'center' },
  imgDash:    { fontSize: 8, color: C.gray },
  // ── Total ─────────────────────────────────────────────────────────────────
  totalSection: { marginTop: 16, alignItems: 'flex-end' },
  totalBox: {
    backgroundColor: C.dark, paddingVertical: 10, paddingHorizontal: 20,
    borderRadius: 6, flexDirection: 'row', alignItems: 'center', gap: 16,
  },
  totalLabel: { fontSize: 11, fontFamily: 'Helvetica-Bold', color: C.white },
  totalValue: { fontSize: 20, fontFamily: 'Helvetica-Bold', color: C.green },
  // ── Note ──────────────────────────────────────────────────────────────────
  noteBox: {
    marginTop: 22, padding: 12, borderLeftWidth: 3, borderLeftColor: C.indigo,
    backgroundColor: '#eff6ff', borderRadius: 4,
  },
  noteLabel: { fontSize: 7, fontFamily: 'Helvetica-Bold', color: C.indigo, marginBottom: 4, letterSpacing: 0.5 },
  noteText:  { fontSize: 9, color: '#1d4ed8' },
  // ── Footer ────────────────────────────────────────────────────────────────
  footer: {
    position: 'absolute', bottom: 28, left: 40, right: 40,
    textAlign: 'center', fontSize: 8, color: C.gray,
      ...
      <Page size="A4" style={styles.page}>
        ...
        {items.length === 0 ? (
          ...
          items.map((item, idx) => {
            ...
            const imgSrc   = proxied(rawImg); // routed through backend proxy

            return (
              <View key={idx} style={[styles.tableRow, idx % 2 === 1 && styles.tableRowAlt]}>

                {/* Imagen vía proxy backend → sin CORS */}
                <View style={styles.colImg}>
                  {imgSrc ? (
                    <Image src={imgSrc} style={styles.productImg} />
                  ) : (
                    <View style={styles.imgBox}>
                      <Text style={styles.imgDash}>—</Text>
                    </View>
                  )}
                </View>
